...
<template>
  <a class="flex overflow-clip flex-col text-xl text-balance bg-gray-800 rounded-2xl m-2 border-gray-300 border-3 shadow-2xl shadow-gray-900" 
    ...
    <div class="flex flex-col md:flex-row items-center md:items-start w-full">
      <video v-if="img.endsWith('.webm') || img.endsWith('.mp4')" class="flex w-full md:max-w-75 md:h-50 md:rounded-bl-xl border-gray-300 border-b-2 md:border-b-0 md:border-r-2" :src="img" :autoplay="true" :loop="true"></video>
      <img v-if="!img.endsWith('.webm') && !img.endsWith('.mp4')" class="flex w-full md:max-w-75 md:h-50 md:rounded-bl-xl border-gray-300 border-b-2 md:border-b-0 md:border-r-2" :src="img">
